...
public final class OAuthFactory<T> extends AuthFactory<String, T> {
    ...
    public T provide() {
        try {
            ...
            } else { //token as queryparam
                String[] accessTokens = request.getParameterMap().get("innlogget_token");

                if(accessTokens != null) {
                    final Optional<T> result = authenticator().authenticate(accessTokens[0]);
                    if (result.isPresent()) {
                        return result.get();
                    }
                }
